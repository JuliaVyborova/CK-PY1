...
def to_csv_file(filename, headers, rows, delimiter=',', new_line='\n'):
    with open(filename, 'w') as f:
        f.write(delimiter.join(headers) + new_line)  # записывается сборка строки заголовков
        f.writelines(delimiter.join(list_) + new_line for list_ in rows)  # построчная запись сборок строк данных
